...
let getThirdPartyNpmLibNames libDir::libDir => {
    ...
    (
      Dep.action_stdout (
        mapD
          ...
          (
            fun () =>
              bashf "./node_modules/jengaboot/buildUtils/extractDeps.out %s" (ts packageJsonPath)
          )
      )
    )
    (
...
let getThirdPartyOcamlfindLibNames libDir::libDir => {
  let packageJsonPath = rel dir::libDir "package.json";
  mapD
    (
      Dep.action_stdout (
        mapD
          ...
          (
            fun () =>
              bashf
                "./node_modules/jengaboot/buildUtils/extractOcamlfindDeps.out %s"
                (ts packageJsonPath)
          )
      )
    )
...
let sortedTransitiveThirdPartyNpmLibs =
  bindD
    (getThirdPartyNpmLibNames libDir::root)
    (
      fun thirdPartyLibNames => {
        ...
        let thirdPartiesThirdPartyLibNamesD = Dep.all (
          List.map thirdPartyLibDirs f::(fun libDir => getThirdPartyNpmLibNames libDir::libDir)
        );
        mapD
...
let transitiveThirdPartyOcamlfindLibs =
  bindD
    (getThirdPartyNpmLibNames libDir::root)
    (
      fun thirdPartyLibNames => {
        ...
        let thirdPartiesThirdPartyOcamlfindLibNamesD = Dep.all (
          List.map
            thirdPartyLibDirs f::(fun libDir => getThirdPartyOcamlfindLibNames libDir::libDir)
        );
        mapD
          thirdPartiesThirdPartyOcamlfindLibNamesD
          (
            fun thirdPartiesThirdPartyOcamlfindLibNames =>
              List.concat thirdPartiesThirdPartyOcamlfindLibNames |> List.dedup
          )
      }
    );
...
let compileSourcesScheme
  ...
  let compileEachSourcePath path =>
    mapD
      (
        Dep.both
          (getThirdPartyNpmLibNames libDir::libDir)
          (
            Dep.both
              (ocamlDepCurrentSources sourcePath::path)
              (getThirdPartyOcamlfindLibNames libDir::libDir)
          )
      )
      (
        fun (thirdPartyNpmLibNames, (firstPartyDeps, thirdPartyOcamlfindLibNames)) => {
...
let finalOutputsScheme sortedSourcePaths::sortedSourcePaths => {
  ...
  Scheme.dep (
    mapD
      (Dep.both sortedTransitiveThirdPartyNpmLibs transitiveThirdPartyOcamlfindLibs)
      (
        fun (sortedTransitiveThirdPartyNpmLibs', transitiveThirdPartyOcamlfindLibs') => {
          let transitiveCmaPaths =
            List.map
              sortedTransitiveThirdPartyNpmLibs'
              f::(fun (Lib name) => rel dir::(rel dir::buildDirRoot name) libraryFileName);
          let ocamlfindPackagesStr =
            if (transitiveThirdPartyOcamlfindLibs' == []) {
              ""
            } else {
              "-linkpkg -package " ^ (
                List.map transitiveThirdPartyOcamlfindLibs' f::tsl |> String.concat sep::","
              )
            };
          let action =
            bashf
...
|}
  ...
  Scheme.rules [
    Rule.create
      targets::[dotMerlinPath] (mapD (getThirdPartyOcamlfindLibNames libDir::dir) saveMerlinAction)
  ]
};
