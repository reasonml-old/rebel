...
let intermediateBinaryOutput =
  rel dir::(rel dir::buildDirRoot (tsl topLibName)) "appIntermediate.out";
        ...
        fun (
          ...
          let action =
            bashf
              ...
              (ts intermediateBinaryOutput)
              (transitiveCmaPaths |> List.map f::ts |> String.concat sep::" ")
              (ts moduleAliasCmoPath)
              cmosString;
          let finalOutputFileContent = Printf.sprintf {|# This is a wrapper around simply calling appIntermediate.out, our pseudo-final
# OCaml bytecode binary output. However, we can't directly call
# `./appIntermediate.out`, because it'll be executed with the `ocamlrun` (the
# runtime), whose path is hard-coded inside the binary as
...
# `ocamlrun ./appIntermediate`, while assuming that `ocamlrun` is currently
# globally exposed on the user's machine (comes with the OCaml distribution).

# Assume this app.out is alongside appIntermediate.out. Find and execute it in
# the same directory as app.out. http://stackoverflow.com/a/17744637/960178
# OCaml binary output. However, we can't directly call that
dir=$(cd -P -- "$(dirname -- "$0")" && pwd -P)
ocamlrun ${dir}/appIntermediate.out
          |};
          Scheme.rules [
            Rule.simple
              targets::[intermediateBinaryOutput]
              deps::(
            ...
            Rule.simple
              ...
              deps::[]
              action::(Action.save finalOutputFileContent target::binaryOutput),
            Rule.simple
              targets::[jsOutput]
              deps::[Dep.path intermediateBinaryOutput]
              action::(
                bashf
    ...
    Scheme.all [
      ...
      Scheme.rules [
        Rule.default
          dir::dir
          [
            Dep.path intermediateBinaryOutput,
