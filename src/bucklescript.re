  ...
  let compilePathScheme path::path (firstPartyDeps, npmPkgs) => {
    ...
    let action =
      bashf
        (
          if isInterface' {
            "%s bsc.exe -g %s %s -pp 'refmt --print binary' -bs-package-name self -bs-package-output commonjs:%s %s %s -o %s -c -intf %s 2>&1; (exit ${PIPESTATUS[0]})"
          } else if (
            hasInterface' && String.is_suffix (Path.basename path) suffix::".re"
          ) {
            "%s bsc.exe -g %s %s -pp 'refmt --print binary' -bin-annot -bs-package-name self -bs-package-output commonjs:%s %s %s -o %s -c -intf-suffix .rei -impl %s 2>&1; (exit ${PIPESTATUS[0]})"
          } else {
            "%s bsc.exe -g %s %s -pp 'refmt --print binary' -bin-annot -bs-package-name self -bs-package-output commonjs:%s %s %s -o %s -c -impl %s 2>&1; (exit ${PIPESTATUS[0]})"
          }
        )
